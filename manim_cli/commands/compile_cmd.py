...
@click.option("--pacing", "pacing_profile", type=click.Choice(["preserve", "teaching", "accelerated"]), default="preserve", show_default=True)
@click.option("--no-cache", is_flag=True, help="Disable compile cache reuse.")
def compile_cmd(scene_json: Path, out_dir: Path, profile: str, pacing_profile: str, no_cache: bool) -> None:
    print_json(compile_scene_file(scene_json, out_dir, profile=profile, use_cache=not no_cache, pacing_profile=pacing_profile))

...
def estimate_bboxes(scene: SceneDef) -> Dict[str, BBoxEstimate]:
...
def estimate_bbox(scene: SceneDef, mob: MobjectDef) -> Optional[BBoxEstimate]:
    ...
    if mob.type in ("Text", "Tex"):
        ...
        method = "text_len_font_heuristic" if mob.type == "Text" else "tex_static_unknown_heuristic"
...
def slot_region(scene: SceneDef, slot: str) -> BBox:
    ...
    regions = {
        ...
        "bottom_formula": BBox(-width * 0.39, -height * 0.4, width * 0.39, -height * 0.26),
        "caption": BBox(-width * 0.39, -height * 0.49, width * 0.39, -height * 0.37),
    ...
    for step in build_timeline(scene):
        ...
        for left_id, right_id in pairs(visible):
            ...
            if pair_conf == "unknown_static":
                warnings.append({"type": "layout_needs_visual_qa", "step": step.step_id, "objects": [left_id, right_id], "bbox_confidence": pair_conf, "bbox_methods": [left.method, right.method]})
            elif pair_conf == "high":
